...
class App:
    ...
    def run(self,one):
        ...
        while True:
                ...
                if(1):
                           ...
                           if(coun == coun_max+1):
                               while(one==0):
                                   ...
                                   win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN,int(screenWidth - 4*x),int(2*y),0,0)
                                   win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP,int(screenWidth - 4*x),int(2*y),0,0)
                                   end = 1
                                   one=1
                           if(coun == coun_max-1):
                               while(one==0):
                                   (x,y) = win32api.GetCursorPos()
                                   win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN,int(screenWidth - 4*x),int(2*y),0,0)
                           ...
                           if(coun == coun_max-2):
                               while(one==0):
                                   (x,y) = win32api.GetCursorPos()
                                   win32api.mouse_event(MOUSEEVENTF_WHEEL, screenWidth-4*x, 2*y, 10, 0)
                                   end=1
                                   one=1
                           if(coun == coun_max):
                               end = 0
            ...
            vis = self.frame.copy()
            for tracker in self.trackers:pyautogui.dragTo(x, y, duration=num_seconds)
            ...
            ch = cv2.waitKey(10)pyautogui.dragTo(x, y, duration=num_seconds)
            if ch == ord('s'):
                coun_max = counpyautogui.dragTo(x, y, duration=num_seconds)
                a=cx-80
                b=cy-80
                c=cx+80
                d=cy+80
