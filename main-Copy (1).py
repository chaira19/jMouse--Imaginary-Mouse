...
class MOSSE:
    ...
    def draw_state(self, vis, i):
        ...
        if self.good:
            ...
            win32api.SetCursorPos((int(screenWidth/2-2*x),int(2*y)))
            i=0
                ...
                if(1):
                           ...
                           if(coun == coun_max+1):
                               while(one==0):
                                   ...
                                   win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTDOWN,int(screenWidth/2-2*x),int(2*y),0,0)
                                   win32api.mouse_event(win32con.MOUSEEVENTF_RIGHTUP,int(screenWidth/2-2*x),int(2*y),0,0)
                                   end = 1
                                   one=1
                           if(coun == coun_max-1):
                               while(one==0): 
                                   (x,y) = win32api.GetCursorPos()
                                   win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN,int(screenWidth/2-2*x),int(2*y),0,0)
                                   win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP,int(screenWidth/2-2*x),int(2*y),0,0)
                                   end = 1
                                   one=1
